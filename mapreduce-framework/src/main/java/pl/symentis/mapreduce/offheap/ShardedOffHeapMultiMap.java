...
public class ShardedOffHeapMultiMap<K, V> implements AutoCloseable {
    ...
    public ShardedOffHeapMultiMap(Path baseDirectory, 
    ...
    public ShardedOffHeapMultiMap(Path baseDirectory,
                                 ...
                                 int shardCount,
                                 long initialSizePerShard) throws IOException {
        ...
        for (int i = 0; i < shardCount; i++) {
            ...
            this.shards[i] = new OffHeapShard<>(dataPath, keyIndexPath, keySerializer, valueSerializer, initialSizePerShard);
        }
    }
    ...
    public ValueIterator<V> getValues(K key) throws IOException, ClassNotFoundException {
        int shardIndex = hashFunction.apply(key);
    ...
    private class AllKeysIterator implements Iterator<K> {
        ...
        public AllKeysIterator() throws IOException, ClassNotFoundException {
        ...
        private void moveToNextNonEmptyShard() throws IOException, ClassNotFoundException {
            while (currentShardIndex < shardCount) {
    ...
    private static class OffHeapShard<K, V> implements AutoCloseable {
        ...
        private static final int DATA_ENTRY_HEADER_SIZE = KEY_HASH_SIZE + KEY_LENGTH_SIZE + VALUE_LENGTH_SIZE + NEXT_POINTER_SIZE;
        ...
        public OffHeapShard(Path dataFilePath, 
            ...
            this.dataChannel = FileChannel.open(dataFilePath,
                StandardOpenOption.CREATE,
                StandardOpenOption.READ,
                StandardOpenOption.WRITE);
            
            this.keyIndexChannel = FileChannel.open(keyIndexFilePath,
                StandardOpenOption.CREATE,
                StandardOpenOption.READ,
                StandardOpenOption.WRITE);
        ...
        private void loadKeyIndex() throws IOException {
            try {
        ...
        private long findMatchingEntry(K targetKey, int targetKeyHash, long startPosition) 
    ...
    public static class ValueIterator<V> implements Iterator<V> {
        ...
        @SuppressWarnings("unchecked")
        private ValueIterator(Object key, int keyHash, long startPosition,
